...
unsigned int max_in_gap(const std::vector<std::pair<uint, std::string>>& male_houses_taxis, const std::vector<std::pair<uint, std::string>>& female_houses_taxis, int x1, int x2, int y1, int y2) {
    uint max = 0;
...
int main() {
    ...
    std::vector<std::vector<std::pair<uint, std::pair<int, int>>>> dp(n + 1, std::vector<std::pair<uint, std::pair<int, int>>>(m + 1,std::make_pair(0, std::make_pair(0, 0))));
    for (int i = 0; i < n + 1; ++i) {
        for (int j = 0; j < m + 1; ++j) {
            if (i * 3 < j) {
                dp[i][j].first = -1;
